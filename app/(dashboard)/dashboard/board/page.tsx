	...
	const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (res.status === 201) {
				...
				setErrorMessage("Performance already exists.");
			} else {
				throw new Error(result.error || res.statusText);
			}
		} catch (error) {
			console.error(error);
			setErrorMessage("Network error: Unable to submit the form.");
		}
	};
								...
								{performances.length > 0 ? (
									performances.map((performance, index) => (
										<div key={index} className="p-3 border-t grid grid-cols-3">
											...
											<div>
												{formatMMDDYYYY(performance.performance_datetime)}
											</div>
											<div>
												<button className="text-jujube hover:underline">
													Edit
												</button>
											</div>
										</div>
